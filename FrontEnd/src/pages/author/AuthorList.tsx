...
const AuthorList : React.FC = () => {
    ...
    useEffect(() => {
        ...
        setField([
            ...
            {key: "Birth", label: "Day of Birth", class: ""},
            {key: "Biography", label: "Biography", class: ""},
    ...
    const handleComponentAdd: React.FC =() => {
        return  (<AuthorAdd loadDataTable={loadDataTable} form={'add'}/>)
    }
    const handleComponentEdit: React.FC =( ) => {
        return  (<AuthorAdd loadDataTable={loadDataTable} form={'edit'} id={currentId} />)
    }
    const handleComponentDetail: React.FC = () => {
        return  (<AuthorDetail  id={currentId} />)
    }
    const showModalAdd = () => {
        dispatch(setShowModal(true))
        setComponentTitle("Add Author")
        setShowComponent('add')
    }
    const showModalEdit = (item: any) => {
        setCurrentId(item.Id)
        setComponentTitle("Edit Author")
        setShowComponent('edit')
        dispatch(setShowModal(true))
    }
    const showModalDetail = (item: any) => {
        setCurrentId(item.Id)
        setComponentTitle("Detail Author")
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $axios.delete(`Author/${item.Id}`).then(res => {
                    ...
                    loadDataTable()
                    dispatch(setToast({status: 'success', message: 'Success', data: 'Delete Author successful'}))
...
const exportToCSV = () => {
    try {
        ...
        console.error(err);
        dispatch(setToast({status: 'error', message: 'Export failed', data: 'Failed to export CSV file'}));
    }
}
                                        ...
                                        <TableRow key={crypto.randomUUID()}>
                                            {
                                                field.map((field: any) => {
                                                    ...
                                                    if (field.key === "Birth"){
                                                        return (
                                                            <TableCell className={field.class} key={crypto.randomUUID()}>
                                                            {item[field.key] ? dayjs(item[field.key]).format("DD-MM-YYYY") : '-'}
                                                        </TableCell>
                                                        )
                                                    }
                                                    return (
